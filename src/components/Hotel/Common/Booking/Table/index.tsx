                    ...
                    <tbody>
                        {
                            isFetching ?
                                <tr >
                                    ...
                                    {
                                        data && data.data && data.data.map((booking) => {
                                            return (
                                                <tr key={booking._id} className="even:bg-primary/10 odd:bg-theme-black dark:even:bg-primary/10 dark:odd:bg-theme-black hover:bg-primary/30 dark:hover:bg-primary/30 group [&:not(:last-child)]:border-b  dark:[&:not(:last-child)]:border-theme-gray [&:not(:last-child)]:border-theme-gray" >
                                                    <td className="px-4 py-2 group-last:first:rounded-bl-xl group-last:last:rounded-br-xl">
                                                        <div className="flex gap-3 items-center">
                                                            <Image src={booking && booking.usersRef && booking.usersRef.profilePic
                                                                && booking.usersRef.profilePic.small ? booking.usersRef.profilePic.small : DefaultProfilePic} width={50} height={50} alt="" className="h-10 w-10 rounded-full" />
                                                            <div>
                                                                <p className="text-white/60 dark:text-white/60">
                                                                    #{booking && booking.bookingID}
                                                                </p>
                                                                <h5 className="font-medium text-base text-white dark:text-white">
                                                                    {booking && booking.usersRef && booking.usersRef.name}
                                                                </h5>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="px-1 py-2 group-last:first:rounded-bl-xl group-last:last:rounded-br-xl">
                                                        <p className="text-white dark:text-white text-sm">
                                                            {booking && booking.usersRef && (booking.usersRef.dialCode + ' ' + booking.usersRef.phoneNumber)}
                                                        </p>
                                                    </td>
                                                    <td className="px-1 py-2 group-last:first:rounded-bl-xl group-last:last:rounded-br-xl">
                                                        <p className="text-white dark:text-white text-sm">
                                                            {booking && booking.createdAt ? moment(booking.createdAt).format('MM-DD-YYYY') : null}
                                                        </p>
                                                    </td>
                                                    <td className="px-1 py-2 group-last:first:rounded-bl-xl group-last:last:rounded-br-xl">
                                                        <p className="text-white dark:text-white text-sm">
                                                            {booking && booking.checkIn ? moment(booking.checkIn).format('MM-DD-YYYY') : null}
                                                        </p>
                                                    </td>
                                                    <td className="px-1 py-2 group-last:first:rounded-bl-xl group-last:last:rounded-br-xl">
                                                        <p className="text-white dark:text-white text-sm">
                                                            {booking && booking.checkOut ? moment(booking.checkOut).format('MM-DD-YYYY') : null}
                                                        </p>
                                                    </td>

                                                    <td className="px-1 py-2 group-last:first:rounded-bl-xl group-last:last:rounded-br-xl">
                                                        <p className="text-white dark:text-white text-sm capitalize">
                                                            {booking && booking.roomsRef && booking.roomsRef.roomType}
                                                        </p>
                                                    </td>
                                                    <td className=" px-1 py-2 group-last:first:rounded-bl-xl group-last:last:rounded-br-xl">
                                                        <p className="text-white dark:text-white text-xs font-semibold">
                                                            {moment(booking.createdAt).format('ddd DD, MMM YYYY hh:mm:ss A')}
                                                        </p>
                                                        ...
                                                        <div className="w-40 relative">
                                                            {
                                                                ['created', 'pending',].includes(booking.status) ? <>
                                                                    <StatusButton status={booking.status} isToggled={toggleIndex === booking._id} onClick={() => toggleIndex === booking._id ? setToggleIndex("") : setToggleIndex(booking._id)} />
                                                                    <StatusDropDown open={booking._id === toggleIndex} onStatusChange={(status) => handleStatusChange(status, booking._id)} />
                                                                </> :
                                                    ...
                                                    <td className=" px-1 py-2 group-last:first:rounded-bl-xl group-last:last:rounded-br-xl">
                                                        <div className="flex gap-2 justify-end pe-2">
                                                            <Button.Hotel.View name="View" svg={<SVG.Eye />} onClick={() => router.push(`/hotels/booking-management/${booking?._id}`)} />
                                                        </div>
                                                    </td>
                                                </tr>
                                            )
