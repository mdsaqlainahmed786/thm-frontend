...
async function getBusinessQr(businessID: string): Promise<any> {
    try {
        const response = await apiRequest.get(`/admin/business/${businessID}/generate-qr`);
...
const Profile: React.FC<{ user: UserProfile | undefined, loading: boolean, edit: boolean }> = ({ loading, user, edit }) => {
    ...
    if (user?.accountType === "business") {
        ...
        if (user && user.businessProfileRef && user.businessProfileRef.phoneNumber && user.businessProfileRef.dialCode) {
        ...
        if (user && user.businessProfileRef.amenitiesRef && user.businessProfileRef) {
            amenities = user.businessProfileRef.amenitiesRef;
        }
        if (user && user.businessProfileRef && user.businessProfileRef.businessTypeRef) {
            businessType = user.businessProfileRef.businessTypeRef.name;
        }
        if (user && user.businessProfileRef && user.businessProfileRef.businessSubtypeRef) {
    ...
    useEffect(() => {
        ...
        if ((session && session.user && session.user._id) && (user && user._id) && session.user._id === user._id) {
            setProfile(true);
        }
        if (user && user.businessProfileID && user.accountType === "business") {
            getBusinessQr(user.businessProfileID).then((data) => {
    ...
    const handleFormSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
        try {
            e.preventDefault();
            const response = await apiRequest.put(`/admin/users/${user?._id}`, formInputs);
            if (response.status === 200 && response.data.status) {
                const responseData = response.data.data;
                const responseMessage = responseData.message;
                toast.success(responseMessage ?? "User updated");
                //Update session data 
                if (profile && responseData && responseData.name && responseData.profilePic) {
                    update({ name: responseData.name, profilePic: responseData.profilePic });
                }
                setModal(false);
            } else {
    ...
    const handleProfileChange: React.ChangeEventHandler<HTMLInputElement> = async (e) => {
        try {
            ...
            const response = await apiRequest.post(`/user/edit-profile-pic`,
                newFromData,
                {
                    headers: {
    ...
    return (
        <>
            <div className={`overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark ${loading ? "animate-pulse" : null} `}>
                {/* Edit User Model */}
                <div className={`${modal ? '' : 'hidden'} fixed left-0 top-0 z-999999 flex h-screen w-full justify-center overflow-y-scroll bg-black/80 px-4 py-5`}>
                    <div className="relative m-auto w-full max-w-180 rounded-sm border border-stroke bg-gray p-4 shadow-default dark:border-strokedark dark:bg-meta-4 sm:p-8 xl:p-10">
                        ...
                        <form onSubmit={handleFormSubmit}>
                            <div className="mb-3">
                                <label htmlFor="name" className="mb-1 block font-medium text-black dark:text-white">Name</label>
                            ...
                            <div className="mb-3">
                                <label htmlFor="bio" className="mb-1 block font-medium text-black dark:text-white">Bio</label>
                                <textarea name="bio" id="bio" cols={30} rows={4} placeholder="Enter task description" className="w-full rounded-sm border border-stroke bg-white px-4.5 py-3 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-boxdark dark:text-white dark:focus:border-primary" value={formInputs.bio} onChange={(e) => setFormInputs({ ...formInputs, bio: e.target.value })}></textarea>
                            </div>
                            <div className="mb-3">
                                <label htmlFor="taskList" className="mb-1 block font-medium text-black dark:text-white">Login Credential</label>
                                <div className="flex flex-col gap-2.5">
                                    <div className="flex items-center gap-2.5">
                                        ...
                                        <input id="email" placeholder="Enter list text" className="w-full rounded-sm border border-stroke bg-white px-4.5 py-3 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-boxdark dark:text-white dark:focus:border-primary disabled:opacity-60" type="text" name="email" disabled={true} value={formInputs.email} />
                                    </div>
                                    <div className="flex items-center gap-2.5">
                                        <div className="flex h-12.5 w-12.5 items-center justify-center rounded-sm border border-stroke bg-white p-4 hover:text-primary dark:border-strokedark dark:bg-boxdark">
                                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15.9655 12.6929C15.672 12.5811 15.5252 12.5251 15.3708 12.5118C15.2164 12.4984 15.0622 12.5283 14.7538 12.5881L12.3984 13.0445C12.0235 13.1172 11.836 13.1535 11.6427 13.1227C11.4493 13.0918 11.2957 13.0064 10.9886 12.8356C9.07557 11.7718 7.78657 10.5439 6.95042 8.81181C6.827 8.55614 6.76529 8.4283 6.74183 8.25256C6.71837 8.07683 6.74863 7.91452 6.80915 7.5899L7.27298 5.1021C7.32932 4.79993 7.35749 4.64884 7.3442 4.49766C7.33092 4.34649 7.27683 4.20263 7.16866 3.91491L6.61323 2.43758C6.35178 1.74217 6.22105 1.39447 5.93618 1.19723C5.65132 1 5.27985 1 4.53692 1H2.68622C1.66783 1 0.866113 1.84144 1.01869 2.8481C1.39788 5.36068 2.51739 9.91727 5.78835 13.1882C9.22363 16.6235 14.172 18.1141 16.895 18.7072C17.9468 18.9357 18.8794 18.1159 18.8794 17.0388V15.3317C18.8794 14.592 18.8794 14.2222 18.6836 13.938C18.4877 13.6538 18.1421 13.5222 17.4509 13.2588L15.9655 12.6929Z" className="fill-current" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"></path></svg>
                                        </div>
                                        <input id="dialCode" placeholder="+91" className="w-12.5 rounded-sm border border-stroke bg-white px-2 py-3 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-boxdark dark:text-white dark:focus:border-primary disabled:opacity-60" type="text" name="dialCode" value={formInputs.dialCode} disabled={true} />
                                        <input id="phoneNumbers" placeholder="Enter phone number" className="w-full rounded-sm border border-stroke bg-white px-4.5 py-3 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-boxdark dark:text-white dark:focus:border-primary disabled:opacity-60" type="text" name="phoneNumbers" value={formInputs.phoneNumber} disabled={true} />
                                    </div>
                                </div>
                            </div>
                            {/* Business Profile  */}
                            {
                                user && user.accountType === "business" ?
                                    <div className="mb-3">
                                        <label htmlFor="taskList" className="mb-1 block font-medium text-black dark:text-white">Business Contact</label>
                                        <div className="flex flex-col gap-2.5">
                                            <div className="flex items-center gap-2.5">
                                                <div className="flex h-12.5 w-12.5 items-center justify-center rounded-sm border border-stroke bg-white p-4 hover:text-primary dark:border-strokedark dark:bg-boxdark">
                                                    <svg className="fill-current" width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15.7499 2.75208H2.2499C1.29365 2.75208 0.478027 3.53957 0.478027 4.52395V13.6364C0.478027 14.5927 1.26553 15.4083 2.2499 15.4083H15.7499C16.7062 15.4083 17.5218 14.6208 17.5218 13.6364V4.49583C17.5218 3.53958 16.7062 2.75208 15.7499 2.75208ZM15.7499 4.0177C15.778 4.0177 15.8062 4.0177 15.8343 4.0177L8.9999 8.4052L2.16553 4.0177C2.19365 4.0177 2.22178 4.0177 2.2499 4.0177H15.7499ZM15.7499 14.0865H2.2499C1.96865 14.0865 1.74365 13.8615 1.74365 13.5802V5.2552L8.3249 9.47395C8.52178 9.61457 8.74678 9.67083 8.97178 9.67083C9.19678 9.67083 9.42178 9.61457 9.61865 9.47395L16.1999 5.2552V13.6083C16.2562 13.8896 16.0312 14.0865 15.7499 14.0865Z" fill=""></path></svg>
                                                </div>
                                                <input id="email" placeholder="Enter list text" className="w-full rounded-sm border border-stroke bg-white px-4.5 py-3 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-boxdark dark:text-white dark:focus:border-primary disabled:opacity-60" type="text" name="email" disabled={true} value={formInputs.businessEmail} />
                                            ...
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <input id="dialCode" placeholder="+91" className="w-12.5 rounded-sm border border-stroke bg-white px-2 py-3 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-boxdark dark:text-white dark:focus:border-primary disabled:opacity-60" type="text" name="dialCode" value={formInputs.businessDialCode} disabled={true} />
                                                <input id="phoneNumbers" placeholder="Enter phone number" className="w-full rounded-sm border border-stroke bg-white px-4.5 py-3 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-boxdark dark:text-white dark:focus:border-primary disabled:opacity-60" type="text" name="phoneNumbers" value={formInputs.businessPhoneNumber} disabled={true} />
                                            </div>
                                        </div>
                                    </div>
                                    : null
                            ...
                            {
                                !profile && <div className="mb-5">
                                    <div className="grid grid-cols-4">
                                        <div>
                                            <label htmlFor="isVerified" className="mb-1 block font-medium text-black dark:text-white">Verified</label>
                                            <div>
                                                <SwitcherThree id="isVerified" enabled={formInputs.isVerified} setEnabled={(e) => setFormInputs({ ...formInputs, isVerified: !formInputs.isVerified })} />
                                            </div>
                                        </div>
                                        <div>
                                            <label htmlFor="isApproved" className="mb-1 block font-medium text-black dark:text-white">Approved</label>
                                            <div>
                                                <SwitcherThree id="isApproved" enabled={formInputs.isApproved} setEnabled={(e) => setFormInputs({ ...formInputs, isApproved: !formInputs.isApproved })} />
                                            </div>
                                        </div>
                                        <div>
                                            <label htmlFor="isActivated" className="mb-1 block font-medium text-black dark:text-white">Activated</label>
                                            <div>
                                                <SwitcherThree id="isActivated" enabled={formInputs.isActivated} setEnabled={(e) => setFormInputs({ ...formInputs, isActivated: !formInputs.isActivated })} />
                                            </div>
                                        </div>
                                        <div>
                                            <label htmlFor="isDeleted" className="mb-1 block font-medium text-black dark:text-white">Delete</label>
                                            <div>
                                                <SwitcherThree id="isDeleted" enabled={formInputs.isDeleted} setEnabled={(e) => setFormInputs({ ...formInputs, isDeleted: !formInputs.isDeleted })} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="mb-3">
                                <label htmlFor="bio" className="mb-1 block font-medium text-black dark:text-white">Role</label>
                                <div className="relative z-20 bg-white dark:bg-form-input">
                                    <select disabled={['administrator', 'official'].includes(formInputs.role)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent px-4 py-3 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input text-black dark:text-white" required={true} value={formInputs.role} onChange={(e) =>
                                        ...
                                        {['administrator', 'official'].includes(formInputs.role) ?
                                            <>
                                                <option value="administrator" className="text-body dark:text-bodydark">Administrator</option>
                                                <option value="official" className="text-body dark:text-bodydark">Official</option>
                                            </> : null
                                        }
                    ...
                    <div className="mt-4 ">
                        {loading ?
                            ...
                            <h3 className=" mb-1.5 text-2xl font-semibold text-black dark:text-white">
                                ...
                                {user && user.username ? <small className="text-sm tracking-wide text-medium">@{user.username}</small> : ''}
                            </h3>}
                        {loading ?
                    ...
                    <>
                                        ...
                                        <div className="xl:basis-2/3 flex flex-col gap-4">
                                            <div>
                                                ...
                                                <div className="flex gap-3 mb-2">
                                                    <div className="flex gap-1">
                                                        {
                                                            Array(5).fill(0).map((_, index) => {
                                                                return (
                                                                    <span key={index}>
                                                                        <svg className="shrink-0 size-4 fill-meta-6" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                                    ...
                                                    <span className="text-sm font-bold leading-[18px] text-primary">
                                                        {(user.businessProfileRef && user.businessProfileRef?.rating) ? parseFloat(`${user.businessProfileRef?.rating}`).toFixed(1) : 0}
                                                    </span>
                                                </div>
                                                <span className="text-sm dark:text-gray-500 text-gray-500 font-semibold">{businessType} - {businessSubtype}</span>
                                            </div>
                                            <div className="flex flex-wrap gap-2.5">
                                                {amenities && amenities.length !== 0 ?
                                                    amenities.map((amenity: any, index: number) => {
                                                        return (
                                                            <div className="flex flex-col justify-center items-center gap-1.5 rounded-sm border border-stroke bg-white px-3 py-2 shadow-default dark:border-strokedark dark:bg-boxdark" key={index} >
                                                                <img src={amenity?.icon ? amenity?.icon : ''} alt="" className="w-7 h-7" />
                                                                <p className="text-xs font-semibold tracking-wider">{amenity?.name ? amenity?.name : ''}</p>
                                                            </div>
                                                        )
                                                    })
                                                    : null
                                            ...
                                            <ul className="flex flex-col gap-2 min-w-80">
                                                <li className="flex items-start gap-3 justify-start ">
                                                    <span>
                                                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M15.9655 12.6929C15.672 12.5811 15.5252 12.5251 15.3708 12.5118C15.2164 12.4984 15.0622 12.5283 14.7538 12.5881L12.3984 13.0445C12.0235 13.1172 11.836 13.1535 11.6427 13.1227C11.4493 13.0918 11.2957 13.0064 10.9886 12.8356C9.07557 11.7718 7.78657 10.5439 6.95042 8.81181C6.827 8.55614 6.76529 8.4283 6.74183 8.25256C6.71837 8.07683 6.74863 7.91452 6.80915 7.5899L7.27298 5.1021C7.32932 4.79993 7.35749 4.64884 7.3442 4.49766C7.33092 4.34649 7.27683 4.20263 7.16866 3.91491L6.61323 2.43758C6.35178 1.74217 6.22105 1.39447 5.93618 1.19723C5.65132 1 5.27985 1 4.53692 1H2.68622C1.66783 1 0.866113 1.84144 1.01869 2.8481C1.39788 5.36068 2.51739 9.91727 5.78835 13.1882C9.22363 16.6235 14.172 18.1141 16.895 18.7072C17.9468 18.9357 18.8794 18.1159 18.8794 17.0388V15.3317C18.8794 14.592 18.8794 14.2222 18.6836 13.938C18.4877 13.6538 18.1421 13.5222 17.4509 13.2588L15.9655 12.6929Z" className="stroke-primary" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
                                                        </svg>
                                                    </span>
                                                    <b>Phone Number:</b>
                                                    {businessPhoneNumber}
                                                ...
                                                {website
                                                    ...
                                                    <li className="flex items-start gap-3 justify-start ">
                                                        <span>
                                                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><g ><path fill-rule="evenodd" clipRule="evenodd" d="M10.0007 2.50065C5.85852 2.50065 2.50065 5.85852 2.50065 10.0007C2.50065 14.1428 5.85852 17.5007 10.0007 17.5007C14.1428 17.5007 17.5007 14.1428 17.5007 10.0007C17.5007 5.85852 14.1428 2.50065 10.0007 2.50065ZM0.833984 10.0007C0.833984 4.93804 4.93804 0.833984 10.0007 0.833984C15.0633 0.833984 19.1673 4.93804 19.1673 10.0007C19.1673 15.0633 15.0633 19.1673 10.0007 19.1673C4.93804 19.1673 0.833984 15.0633 0.833984 10.0007Z" className="fill-primary"></path><path fill-rule="evenodd" clipRule="evenodd" d="M0.833984 9.99935C0.833984 9.53911 1.20708 9.16602 1.66732 9.16602H18.334C18.7942 9.16602 19.1673 9.53911 19.1673 9.99935C19.1673 10.4596 18.7942 10.8327 18.334 10.8327H1.66732C1.20708 10.8327 0.833984 10.4596 0.833984 9.99935Z" className="fill-primary"></path><path fill-rule="evenodd" clipRule="evenodd" d="M7.50084 10.0008C7.55796 12.5632 8.4392 15.0301 10.0006 17.0418C11.5621 15.0301 12.4433 12.5632 12.5005 10.0008C12.4433 7.43845 11.5621 4.97153 10.0007 2.95982C8.4392 4.97153 7.55796 7.43845 7.50084 10.0008ZM10.0007 1.66749L9.38536 1.10547C7.16473 3.53658 5.90275 6.69153 5.83417 9.98346C5.83392 9.99503 5.83392 10.0066 5.83417 10.0182C5.90275 13.3101 7.16473 16.4651 9.38536 18.8962C9.54325 19.069 9.76655 19.1675 10.0007 19.1675C10.2348 19.1675 10.4581 19.069 10.6159 18.8962C12.8366 16.4651 14.0986 13.3101 14.1671 10.0182C14.1674 10.0066 14.1674 9.99503 14.1671 9.98346C14.0986 6.69153 12.8366 3.53658 10.6159 1.10547L10.0007 1.66749Z" className="fill-primary"></path></g></svg>
                                                        </span>
                                                        <b>Website:</b>
                                                        <Link href={website} target="_blank" children={website} className="text-primary" />
                                                    </li>
                                                    : null
                                                }
                                                <li className="flex items-start gap-3 justify-start ">
                                                    <span>
                                                        <svg width="20" height="20" viewBox="0 0 16 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M14.6654 8.60868C14.6654 11.8081 12.1279 14.3862 10.184 16.6526C9.22437 17.7713 8.74457 18.3307 7.9987 18.3307C7.25282 18.3307 6.77302 17.7713 5.81342 16.6526C3.86945 14.3862 1.33203 11.8081 1.33203 8.60868C1.33203 6.76685 2.03441 5.00046 3.28465 3.69809C4.5349 2.39573 6.23059 1.66406 7.9987 1.66406C9.76681 1.66406 11.4625 2.39573 12.7127 3.69809C13.963 5.00046 14.6654 6.76685 14.6654 8.60868Z" className="stroke-primary" strokeWidth="1.6" />
                                                            <path d="M9.66536 7.7406C9.66536 8.69945 8.91917 9.47676 7.9987 9.47676C7.07822 9.47676 6.33203 8.69945 6.33203 7.7406C6.33203 6.78175 7.07822 6.00445 7.9987 6.00445C8.91917 6.00445 9.66536 6.78175 9.66536 7.7406Z" className="stroke-primary" strokeWidth="1.6" />
                                                        </svg>
                                                    </span>
                                                    <b>Address:</b>
                                                    {businessAddress}
                                        ...
                                        <div className="xl:basis-1/2 flex xl:justify-end items-start">
                                            <div className="flex flex-col justify-center items-center gap-4">
                                                <div className="flex items-center justify-between w-full">
                                                    <h4 className="text-xl font-semibold text-black dark:text-white">Download QR</h4>
                                                    <button className="flex items-center gap-2  bg-primary px-2 py-2 font-medium text-white hover:bg-opacity-80 rounded-full" onClick={downloadQR}>
                                                ...
                                                <div className="w-75 h-75 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                                                    <img src={qr} width={300} height={300} className="w-full h-full" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div className="mt-4 grid grid-cols-1 gap-4 md:mt-6 md:gap-6 2xl:mt-7.5 2xl:gap-7.5">
                            <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                                <div className="mb-4 justify-between sm:flex">
                                    <div className="mb-2">
                                        <h3 className="text-xl font-semibold text-black dark:text-white">Business Documents</h3>
                                    </div>
                                </div>
                                <div className="grid grid-cols-1 gap-5 md:grid-cols-2  xl:gap-10">
                                    {businessDocuments && businessDocuments.length !== 0 ?
                                        businessDocuments.map((document: any, index: number) => {
                                            return (
                                                <div key={index} className="bg-whiten object-cover h-96 w-108 ">
                                                    <p className="text-sm font-semibold my-2 px-3">Business Registration</p>
                                                    <button onClick={() => onDownloadClick(document?.businessRegistration ? document?.businessRegistration : '')} type="button" className="mx-3 my-4 flex w-13 justify-center items-center gap-2  bg-primary px-2 py-2 font-medium text-white hover:bg-opacity-80 rounded-full ">
                                                        <svg className="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M16.8754 11.6719C16.5379 11.6719 16.2285 11.9531 16.2285 12.3187V14.8219C16.2285 15.075 16.0316 15.2719 15.7785 15.2719H2.22227C1.96914 15.2719 1.77227 15.075 1.77227 14.8219V12.3187C1.77227 11.9812 1.49102 11.6719 1.12539 11.6719C0.759766 11.6719 0.478516 11.9531 0.478516 12.3187V14.8219C0.478516 15.7781 1.23789 16.5375 2.19414 16.5375H15.7785C16.7348 16.5375 17.4941 15.7781 17.4941 14.8219V12.3187C17.5223 11.9531 17.2129 11.6719 16.8754 11.6719Z" fill=""> </path>
                                                            <path d="M8.55074 12.3469C8.66324 12.4594 8.83199 12.5156 9.00074 12.5156C9.16949 12.5156 9.31012 12.4594 9.45074 12.3469L13.4726 8.43752C13.7257 8.1844 13.7257 7.79065 13.5007 7.53752C13.2476 7.2844 12.8539 7.2844 12.6007 7.5094L9.64762 10.4063V2.1094C9.64762 1.7719 9.36637 1.46252 9.00074 1.46252C8.66324 1.46252 8.35387 1.74377 8.35387 2.1094V10.4063L5.40074 7.53752C5.14762 7.2844 4.75387 7.31252 4.50074 7.53752C4.24762 7.79065 4.27574 8.1844 4.50074 8.43752L8.55074 12.3469Z" fill=""></path>
                                                        </svg>
                                                    ...
                                                    <iframe
                                                        src={document?.businessRegistration ? document?.businessRegistration : ''}
                                                        height="100%"
                                                        width="100%"
                                                    ></iframe>
                                                </div>
                                    ...
                                    {businessDocuments && businessDocuments.length !== 0 ?
                                        businessDocuments.map((document: any, index: number) => {
                                            return (
                                                <div key={index} className="bg-whiten object-cover h-96 w-108  ">
                                                    <p className="text-sm font-semibold my-2 px-3">Business Address Proof</p>
                                                    <button onClick={() => onDownloadClick(document?.addressProof ? document?.addressProof : '')} className="mx-3 my-4 flex w-13 justify-center items-center gap-2  bg-primary px-2 py-2 font-medium text-white hover:bg-opacity-80 rounded-full ">
                                                        <svg className="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M16.8754 11.6719C16.5379 11.6719 16.2285 11.9531 16.2285 12.3187V14.8219C16.2285 15.075 16.0316 15.2719 15.7785 15.2719H2.22227C1.96914 15.2719 1.77227 15.075 1.77227 14.8219V12.3187C1.77227 11.9812 1.49102 11.6719 1.12539 11.6719C0.759766 11.6719 0.478516 11.9531 0.478516 12.3187V14.8219C0.478516 15.7781 1.23789 16.5375 2.19414 16.5375H15.7785C16.7348 16.5375 17.4941 15.7781 17.4941 14.8219V12.3187C17.5223 11.9531 17.2129 11.6719 16.8754 11.6719Z" fill=""> </path>
                                                            <path d="M8.55074 12.3469C8.66324 12.4594 8.83199 12.5156 9.00074 12.5156C9.16949 12.5156 9.31012 12.4594 9.45074 12.3469L13.4726 8.43752C13.7257 8.1844 13.7257 7.79065 13.5007 7.53752C13.2476 7.2844 12.8539 7.2844 12.6007 7.5094L9.64762 10.4063V2.1094C9.64762 1.7719 9.36637 1.46252 9.00074 1.46252C8.66324 1.46252 8.35387 1.74377 8.35387 2.1094V10.4063L5.40074 7.53752C5.14762 7.2844 4.75387 7.31252 4.50074 7.53752C4.24762 7.79065 4.27574 8.1844 4.50074 8.43752L8.55074 12.3469Z" fill=""></path>
                                                        </svg>
                                                    ...
                                                    <iframe
                                                        src={document?.addressProof ? document?.addressProof : ''}
                                                        height="100%"
                                                        width="100%"
                                                    ></iframe>
                                                </div>
...
const UserDetailedView: React.FC<{ id: string, name: string, accountType: string; username: string, image?: string, register?: boolean }> = ({ id, name, accountType, username, image, register }) => {
    const imageSrc = image || DefaultProfilePic;
    return (
        <Link href={!register ? `/users/${id}` : "#"} className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <div>
                <div className="h-12 w-12 rounded-full bg-black/10 dark:bg-meta-4 flex justify-center items-center">
                    <Image src={imageSrc} alt={name} width={80} height={80} className="w-12 h-12 object-cover rounded-full" />
                </div>
            </div>
            <div>
                ...
                {
                    !register ? <p className="text-xs text-black dark:text-white mb-1">{username}</p> : <p className="text-xs text-meta-1 dark:text-white mb-1">
