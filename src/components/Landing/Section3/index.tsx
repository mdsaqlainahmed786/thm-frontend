...
const Section3 = () => {
 ...
 const [windowWidth, setWindowWidth] = useState(0);
   ...
   const svg1Y = useSpring(
      useTransform(scrollY, [6200, 6400, 6800, 6801], [-1200, 0, 0, -1000]),
      { stiffness: 60, damping: 30 }
   );
   const svg2Y = useSpring(
      useTransform(scrollY, [6200, 6400, 6800, 6801], [-1200, -70, -70, -1000]),
      { stiffness: 60, damping: 30 }
   );

   const svg1X = useSpring(
      useTransform(scrollY, [6200, 6400, 6800, 6801], [-500, -150, -150, -500]),
      { stiffness: 60, damping: 30 }
   );
   ...
   const listY = useSpring(
      useTransform(scrollY, [6200, 6400, 6800, 6801], [1200, -50, -50, -1200]),
      { stiffness: 60, damping: 30 }
   );

   const firstHeadY = useSpring(
      useTransform(scrollY, [5300, 5500], [0, -1200]),
      { stiffness: 50, damping: 30 }
   );

   const secondHeadY = useSpring(
      useTransform(scrollY, [5500, 5700, 6800, 6801], [1200, -50, -50, -1200]),
      { stiffness: 50, damping: 30 }
   );
