"use client";
import { motion, useScroll, useTransform } from "framer-motion";
import { useMotionValueEvent, } from "framer-motion";
import { useEffect, useState } from "react";
import { useSpring } from "framer-motion";
import { physics } from "@/app/(main-app)/landing/SmoothScroll";

const imageList = [
    "/svg/create-post.svg",
    "/svg/explore.svg",
    "/svg/connect.svg",
    "/svg/share-memories.svg",
    "/svg/easy-booking.svg",
    "/svg/rate-review.svg",
    "/svg/earn-reward.svg",
];

const Feature = () => {
    const { scrollY } = useScroll();
    const [prevScroll, setPrevScroll] = useState(0);
    const [activeIndex, setActiveIndex] = useState(0);
    const springY = useSpring(scrollY, physics);
    const [windowWidth, setWindowWidth] = useState(0);

    useEffect(() => {
        setWindowWidth(window.innerWidth);
    }, [windowWidth]);
    function update(latest: number, prev: number): void {
        console.log(latest)
    }

    useMotionValueEvent(scrollY, "change", (latest: number) => {
        // update(latest, prevScroll);
        setPrevScroll(latest);
    });


    useMotionValueEvent(springY, "change", (latest) => {
        const thresholds = [2801, 3101, 3401, 3801, 4101, 4401, 4701];
        const index = thresholds.findLastIndex((v) => latest >= v);
        setActiveIndex(index); // -1 means "none shown"
    });



    const position = [1501, 1900]
    const x = useTransform(springY, position, [400, -400]);

    //Hotel media does more then just rate Done
    const backgroundPosition = useTransform(springY, position, ['0%', '120%']);

    const position2 = [1900, 2100]
    let opacity = useTransform(springY, position2, ["0.0", "1"]);
    // let y = useTransform(springY, position2, [-30, 0]);
    let scale = useTransform(springY, position2, ["0.8", "1"]);



    // x movement: moves from 450px right to center
    let x1 = useTransform(springY, [2100, 2300, 2600], [50, 50, -500]);

    // y movement: moves from -100px up to center (or whatever you want)
    let y1 = useTransform(springY, [2300, 2600], [0, 300]);

    const mobileY1 = useTransform(springY, [2300, 2600], [0, -300]);



    const wordsY = useTransform(
        springY,
        [0, 2600, 2801, 3101, 3401, 3801, 4101, 4401, 4701],
        [700, 700, 460, 388, 316, 245, 170, 100, 27]
    );
    const mobileWordsY = useTransform(
        springY,
        [0, 2600, 2801, 3101, 3401, 3801, 4101, 4401, 4701],
        [700, 700, 380, 330, 260, 200, 130, 100, 30]
    )
    const wordsLineX = useSpring(
        useTransform(scrollY, [5000, 5050], [0, -1200])
        , { stiffness: 70, damping: 30 }
    );

    const iconsX = useSpring(
        useTransform(scrollY, [5000, 5050], [0, 1200])
        , { stiffness: 70, damping: 30 }
    );
    return (
        <div className="relative h-[130dvh] ">
            <motion.div className="flex justify-center items-center text-center p-125 "
                style={{
                    x: wordsLineX,
                }}
            >
                <motion.p className="text-7xl absolute font-bold font-quicksand duration-[1.3s] leading-normal w-[3000px]" style={{
                    overflow: "hidden",
                    background: "linear-gradient(90deg, #000, #fff, #000)",
                    backgroundRepeat: "no-repeat",
                    backgroundSize: "40%",
                    WebkitBackgroundClip: "text",
                    x: x,
                    // scale,
                    backgroundPosition,
                    WebkitTextFillColor: "rgba(255, 255, 255, 0)",
                }} >
                    Hotel Media does more than just rate
                </motion.p>
                <motion.p className="text-5xl absolute font-bold font-quicksand leading-normal text-[#565656]"
                    style={{
                        opacity,
                        x: windowWidth > 600 ? x1 : 0,
                        y: windowWidth > 600 ? y1 : mobileY1,
                        scale,
                    }} >
                    It allows User to
                </motion.p>

                <div
                    className={`absolute overflow-hidden h-[400px] w-[400px] left-[30%] bottom-[180px] md:w-[500px] md:h-[500px] md:top-[330px] md:left-[200px] `}>
                    <motion.div className="flex flex-col gap-8 md:gap-12 text-start h-full ps-4 md:ps-[1px]"
                        style={{
                            y: windowWidth > 600 ? wordsY : mobileWordsY
                        }}
                    >
                        <motion.span
                            className="text-[40px] md:text-[51px] text-primary font-quicksand font-bold"

                        >
                            Create Post
                        </motion.span>

                        <motion.span
                            className="text-[40px] md:text-[51px] text-primary font-quicksand font-bold"

                        >
                            Explore
                        </motion.span>
                        <motion.span
                            className="text-[40px] md:text-[51px] text-primary font-quicksand font-bold"

                        >
                            Connect
                        </motion.span>
                        <motion.span
                            className="text-[40px] md:text-[51px] text-primary font-quicksand font-bold"

                        >
                            Share Memories
                        </motion.span>
                        <motion.span
                            className="text-[40px] md:text-[51px] text-primary font-quicksand font-bold"

                        >
                            Easy booking
                        </motion.span>
                        <motion.span
                            className="text-[40px] md:text-[51px] text-primary font-quicksand font-bold"

                        >
                            Rate and Review
                        </motion.span>
                        <motion.span
                            className="text-[40px] md:text-[51px] text-primary font-quicksand font-bold"

                        >
                            Earn Reward
                        </motion.span>
                    </motion.div>
                </div>


            </motion.div>
            {/* <div className="absolute grid grid-cols-2 gap-16.5 h-100 right-[100px]"> */}

            {activeIndex >= 0 && (
                <motion.div className=" w-[400px] md:w-[520px] h-[260px] md:h-[305px] absolute right-[30%] md:right-[-200px] bottom-[600px] md:bottom-[400px] overflow-hidden"
                    style={{
                        x: iconsX,
                        // y: windowWidth < 600 ? -220 : undefined
                    }}
                >
                    {imageList.map((src, index) => (
                        <motion.img
                            key={index}
                            src={src}
                            initial={{ opacity: 0, y: 50 }}
                            animate={activeIndex === index ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
                            transition={{ duration: 0.5 }}
                            className="absolute inset-0 w-full h-full object-contain will-transform"
                        />
                    ))}
                </motion.div>
            )}

        </div>
    )
}

export default Feature;