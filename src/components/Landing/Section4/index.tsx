...
const Section4 = () => {
   ...
   const firstHeadY = useSpring(
      useTransform(scrollY, [6700, 6750], [1200, 50]),
      { stiffness: 60, damping: 18 }
   );

   const mainHeadY = useSpring(
      useTransform(scrollY, [7100, 7200, 7700, 7701], [0, -200, -200, -1200]),
      { stiffness: 60, damping: 18 }
   );
   const mobileMainHeadY = useSpring(
      useTransform(scrollY, [7100, 7200, 7700, 7701], [0, -90, -90, -1200]),
      { stiffness: 60, damping: 18 }
   );

   const iconsY = useSpring(
      useTransform(scrollY, [7100, 7200], [1000, 150]),
      { stiffness: 60, damping: 18 }
   );

   const iconleftX = useSpring(
      useTransform(scrollY, [7100, 7200], [-1000, 0]),
      { stiffness: 60, damping: 18 }
   );
   ...
   return (
      ...
      <motion.div className="flex flex-col justify-center items-center container"
         style={{
