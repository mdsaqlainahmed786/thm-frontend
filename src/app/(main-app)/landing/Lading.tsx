...
export default function Landing() {
    ...
    const section2Y = useSpring(
        useTransform(scrollY, [1501, 1550], [900, 0])
        , { stiffness: 50, damping: 30 }
    );

    const section3Y = useSpring(
        useTransform(scrollY, [5030, 5100], [1200, 0])
        , { stiffness: 50, damping: 30 }
    );

    const section4Y = useSpring(
        useTransform(scrollY, [6801, 6901], [1200, 0])
        , { stiffness: 50, damping: 30 }
    );
    const section5Y = useSpring(
        useTransform(scrollY, [8600, 8700], [1200, 0])
        , { stiffness: 50, damping: 30 }
    );

    const section6Y = useSpring(
        useTransform(scrollY, [9700, 9800], [-1200, 0])
        , { stiffness: 50, damping: 30 }
    );
