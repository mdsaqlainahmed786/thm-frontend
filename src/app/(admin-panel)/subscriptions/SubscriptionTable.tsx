                                            ...
                                            return (
                                                <tr key={key}>
                                                    <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark gap-1.5">
                                                        <h5 >
                                                            <span className="text-sm font-bold tracking-wide capitalize text-meta-4" title="Order ID">#{data.ordersRef && data.ordersRef.orderID}</span><br />
                                                            <span className="text-xs capitalize font-semibold" title="Razorpay Order Id"> Razorpay:: {data.ordersRef && data.ordersRef.razorPayOrderID}</span><br />
                                                            <span className="text-xs capitalize font-normal" title="Database ID"> DB:: {data.ordersRef && data.ordersRef._id}</span>
                                                        </h5>
                                                    </td>
                                                    <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark xl:pl-2">
                                                        <UserDetailedView
                                                            ...
                                                            image={(data.usersRef && data.usersRef.profilePic && data.usersRef.profilePic.small) ? data.usersRef.profilePic.small : undefined}
                                                        />
                                                    </td>
                                                    <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark xl:pl-2">
                                                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                                                            <div>
                                                                <div className="h-12 w-12 rounded-sm bg-primary  flex justify-center items-center">
                                                                    <Image src={(data.subscriptionPlansRef && data.subscriptionPlansRef.image) ? data.subscriptionPlansRef.image : DefaultProfilePic} alt={data.usersRef && data.usersRef.name} width={60} height={60} className="w-5 h-5 object-cover" />
                                                                </div>
                                                            </div>
                                                            <div>
                                                                <h5 className="font-semibold text-black dark:text-white">
                                                                    {data.subscriptionPlansRef && data.subscriptionPlansRef.name}
                                                                    <small className="text-xs capitalize"> ({data.subscriptionPlansRef && data.subscriptionPlansRef.type})</small>
                                                                </h5>
                                                                <p className="text-xs text-black dark:text-white mb-1 capitalize">
                                                                    {data.subscriptionPlansRef && data.subscriptionPlansRef.currency === "INR" ? '₹' : data.subscriptionPlansRef.currency}
                                                                    {data.subscriptionPlansRef && data.subscriptionPlansRef.price} / {data.subscriptionPlansRef && data.subscriptionPlansRef.duration}
                                                                </p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                                        <div>
                                                            <h5 className="font-semibold text-meta-3 dark:text-white">
                                                                {'₹'} {(data.ordersRef && data.ordersRef.paymentDetail && data.ordersRef.paymentDetail.transactionAmount) ?? 0}
                                                                <small className="text-xs capitalize text-black dark:text-white"> ({data.ordersRef && data.ordersRef.paymentDetail && data.ordersRef.paymentDetail.paymentMethod})</small>
                                                            </h5>
                                                            <p className="text-xs text-black dark:text-white mb-1 capitalize">
                                                                {data.ordersRef && data.ordersRef.paymentDetail && data.ordersRef.paymentDetail.transactionID}
                                                            </p>
                                                        </div>
                                                    </td>
                                                    <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">

                                                        {data.isCancelled ? <span className="text-meta-1 text-sm font-bold tracking-wide">Cancelled</span>
                                                            :
                                                            (data && data.expirationDate && new Date(data.expirationDate) < new Date())
                                                                ? <span className="text-meta-6 text-sm font-bold tracking-wide">Expired</span>
                                                                :
                                                                <span className="text-meta-3 text-sm font-bold tracking-wide">Active</span>
                                                        }
                                                        <p className="text-xs font-medium">
                                                            {moment(data && data.expirationDate).fromNow()}
                                                        </p>
                                                        <p className="text-black dark:text-white text-xs">
                                                            {moment(data && data.expirationDate).format('ddd DD, MMM YYYY hh:mm:ss A')}
                                                        </p>
