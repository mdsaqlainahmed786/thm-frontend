...
const SubscriptionPlansTable = () => {
  ...
  const { isPending, isError, error, data, isFetching, isPlaceholderData, refetch } = useQuery({
    queryKey: ['subscription-plans', query, pageNo],
    queryFn: () => fetchUsers(),
    placeholderData: keepPreviousData,
  });
  const { data: businessTypes, refetch: fetchBusinessTypes } = useQuery({
    queryKey: ['business-types', query, pageNo],
    queryFn: async () => {
      try {
  ...
  const { data: businessSubTypes, refetch: fetchBusinessSubTypes } = useQuery({
    ...
    queryFn: async () => {
      try {
        if (formInputs.businessTypeID && formInputs.businessTypeID !== "") {
          const response = await apiRequest.get(`/business-subtypes/${formInputs.businessTypeID}`, {
            params: {
              query: "",
              documentLimit: 20,
              pageNumber: pageNo
