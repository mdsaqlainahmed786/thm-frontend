...
const ReportsVisualization = () => {
  ...
  const statistics = useMemo(() => {
    ...
    const userReportCounts: { [key: string]: { count: number; name: string } } =
      {};
    reports.forEach((report) => {
      let userId = "";
      ...
      if (report.contentType === "user" && report.usersRef) {
        userId = report.usersRef._id;
        userName =
          report.usersRef.name || report.usersRef.username || "Unknown";
      } else if (report.postsRef) {
        userId = report.postsRef.userID || report.postsRef._id;
        userName = "Post Content";
      } else if (report.commentsRef) {
        userId = report.commentsRef.userID || report.commentsRef._id;
        userName = "Comment Content";
      }

      if (userId) {
        if (!userReportCounts[userId]) {
          userReportCounts[userId] = { count: 0, name: userName };
        }
        userReportCounts[userId].count++;
      }
    });

    const topOffendingUser =
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-5">
        ...
        <CardDataStats
          loading={isFetching}
          title="Top Offending User"
          total={statistics.topOffendingUser}
          rate=""
        >
          <svg
            className="fill-primary dark:fill-white"
