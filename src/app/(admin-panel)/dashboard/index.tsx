...
const CardDataStatsWrapper: React.FC<{}> = () => {
    ...
    return (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5" >
            <Link href={'/users/individual'}>
                <CardDataStats loading={isFetching} title="Total Users" total={data && data.statistics && data.statistics.users ? `${data.statistics.users.count}` : '0'} rate={data && data.statistics && data.statistics.users ? `${data.statistics.users.percentage}%` : '0%'} levelDown={Math.sign(data?.statistics?.users?.percentage ?? 0) === -1} levelUp={Math.sign(data?.statistics?.users?.percentage ?? 0) === +1}>
                    <svg
            ...
            <Link href={'/posts'}>
                <CardDataStats loading={isFetching} title="Total Posts" total={data && data.statistics && data.statistics.posts ? `${data.statistics.posts.count}` : '0'} rate={data && data.statistics && data.statistics.posts ? `${data.statistics.posts.percentage}%` : '0'} levelDown={Math.sign(data?.statistics?.posts?.percentage ?? 0) === -1} levelUp={Math.sign(data?.statistics?.posts?.percentage ?? 0) === +1}>
            ...
            <Link href={'/users/business'}>
                <CardDataStats loading={isFetching} title="Business Profile" total={data && data.statistics && data.statistics.businessProfiles ? `${data.statistics.businessProfiles.count}` : '0'} rate={data && data.statistics && data.statistics.businessProfiles ? `${data.statistics.businessProfiles.percentage}%` : '0%'} levelDown={Math.sign(data?.statistics?.businessProfiles?.percentage ?? 0) === -1} levelUp={Math.sign(data?.statistics?.businessProfiles?.percentage ?? 0) === +1}>
            ...
            <Link href={'/reports'}>
                <CardDataStats loading={isFetching} title="Reported Content" total={data && data.statistics && data.statistics.reports ? `${data.statistics.reports.count}` : '0'} rate={data && data.statistics && data.statistics.reports ? `${data.statistics.reports.percentage}%` : '0'} levelDown={Math.sign(data?.statistics?.reports?.percentage ?? 0) === -1} levelUp={Math.sign(data?.statistics?.reports?.percentage ?? 0) === +1}>
